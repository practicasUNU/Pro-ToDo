...
- **Interpolación de variables en `docker-compose.yml`:** El archivo usa `${DB_USER}`, `${DB_PASSWORD}`, `${DB_PORT}`, `${DB_NAME}`. Docker Compose solo resuelve esa interpolación leyendo un `.env` **junto al propio `docker-compose.yml`** (o vía `--env-file`) — la directiva `env_file:` de un servicio NO sirve para esto, solo inyecta variables dentro del contenedor en runtime. Como la fuente real es `backend/.env`, la raíz mantiene un symlink hacia él (`ln -s backend/.env .env`) en lugar de duplicar el archivo; ese symlink no se versiona (cubierto por el patrón `.env*` en `.gitignore`) y debe recrearse tras cada clonado.
